...
// Lazy load YouTube videos on swiper initialization
document.addEventListener('DOMContentLoaded', function () {
  const iframes = document.querySelectorAll('.swiper-slide iframe');
  
  iframes.forEach(iframe => {
    iframe.src = iframe.dataset.src; // Load all iframe src when the DOM is loaded
    console.log('Loaded iframe:', iframe.src);
  });
});
